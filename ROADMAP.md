...
no voice, no presences. This document tracks what discord.js offers that is
still missing here, and where each item lands across upcoming versions.
...
- [ ] **Autocomplete interactions** (`InteractionType.ApplicationCommandAutocomplete`):
...
- [ ] **Context-menu & primary-entry-point dispatch in `InteractionRouter`**:
...
- [ ] **Wrapped interactions in router dispatch**: handlers currently receive raw
      ...
      `reply`, `deferReply`, ...).
- [ ] **Pattern-based component ids**: `onComponent('config:*', handler)` /
      regex matching so dynamic custom ids don't need one registration each.
- [ ] **Middleware & error hooks**: `router.use(...)` for pre-dispatch work and
      `router.onError(...)` to turn handler exceptions into a safe user-facing reply.
- [ ] **Modal submit getters for new components** on `ModalSubmitInteraction`:
...
- [ ] **Follow-up deletion**: `deleteOriginal()` / `deleteFollowup(messageId)`
      on `InteractionContext` and `DiscordRestClient` (DELETE webhook message endpoints).
- [ ] **MIGRATION.md**: referenced in the changelog but missing from the repo.

## v0.7 — Messages & channels REST surface

- [ ] `fetchMessage`, `fetchMessages({ limit, before, after, around })` with pagination.
- [ ] `bulkDeleteMessages(channelId, ids)` (2–100 messages endpoint).
- [ ] `triggerTyping(channelId)`.
- [ ] Channel management lite: `fetchChannel`, `editChannel` (name/topic/slowmode/nsfw),
      `deleteChannel`, `followAnnouncementChannel`.
- [ ] Reaction management: list reactors (`GET .../reactions/{emoji}`), remove own/all reactions.
- [ ] **Polls**: poll object in send options, `endPoll(channelId, messageId)`,
      fetch answer voters — fully HTTP-compatible, discord.js parity feature.
- [ ] Attachment/CDN helpers: build attachment URLs, avatar/banner/icon URL builders
      (discord.js `CDN` class equivalents).

## v0.8 — Guilds & moderation REST

- [ ] Members: `fetchMember`, `kick`, `ban`/`unban` (+ ban list), `timeout`
      (communication_disabled_until), role add/remove.
- [ ] Roles: create/edit/delete/list, position reordering.
- [ ] Guild basics: fetch guild, list channels, list members (paginated).
- [ ] Emoji & sticker CRUD.
- [ ] Webhook management: create/list/delete channel webhooks; execute incoming
      webhooks with components (`with_components=true`).
- [ ] **Entitlements & SKUs**: list/consume entitlements — pairs with the existing
...
- [ ] **Rate-limit bucket handling**: parse `X-RateLimit-*` headers, serialise per
...
- [ ] Snowflake utilities: `snowflakeToTimestamp`, id generation checks.
- [ ] Test suite + CI: unit tests for builders/payload normalisation (pure functions),
      GitHub Actions running typecheck/build/tests, npm publish workflow on version tags.
- [ ] Localisation ergonomics: typed locale-key maps shared across commands/options/embeds.
- [ ] Interaction response timeout guard as a first-class option on every handler.
